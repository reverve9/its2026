...
//
// 등급이 둘이다:
//   superAdmin  운영본부 — 사이드바 전체(운영·콘텐츠·정산)
//   client      발주처(강릉시) — 대시보드 + '운영' 섹션만. 콘텐츠·정산은 항목이 전부 superAdminOnly.
//
// 콘텐츠·정산을 통째로 가리는 이유: 운영본부의 내부 대장·발행 원고다. 인력 현황은 명부(연락처·
// 외국어·물품 지급·정산 서류), 업체 등록 현황은 입점 관리, 정산은 우리 원가(일용 지급·원천징수),
// 만족도조사/프로그램/공지FAQ/쿠폰은 발행 전 관리 화면이다. 발주처가 볼 것은 현황(대시보드·운영)이다.
// 즉 등급은 권한이 아니라 '보고 경계'를 화면으로 옮긴 것이다. 실제 가림은 consoleNav 의
// superAdminOnly 가 단일 출처(D46) — 이 주석은 그 경계의 설명일 뿐이다.
