...
//
// ⚠️ 작업중지는 이 플랫폼에 아예 없다 — 콘솔에도 없다. 되살리지 말 것.
// 과업지시서는 '작업중지'와 '운영중단'을 나란히 다른 조치로 쓰지만, 작업중지의 대상인
// 위험작업(고소·전기·중량물·구조물·야간)은 설치·철거에 몰려 있어 이 플랫폼이 다루는
// 행사 5일(10/19~23, 프로덕션) 밖이다. 게다가 우리가 발령하는 것도 아니다 — 과업지시서는
// 발주기관·관계기관이 요구하면 즉시 이행할 의무로 규정한다. 모델링 대상이 아니었다.
//
...
export function SafetyBanner({ assignmentId }: { assignmentId?: string | null }) {
  ...
  // 거점을 모르는 경우(로그인 전 · 슈퍼어드민)엔 전 거점 발령만 띄운다.
