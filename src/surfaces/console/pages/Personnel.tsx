...
import type { EducationKind, PersonnelRecord, Shift, StaffRole } from '../../../types'
...
const roleFilters: { key: StaffRole | 'all'; label: string }[] = [
  { key: 'all', label: '전체 역할' },
  { key: '거점관리자', label: '거점관리자' },
  { key: '봉사자', label: '봉사자' },
]
...
export default function Personnel() {
  ...
  const langCount = people.filter((p) => p.lang.length > 0).length
  ...
  // 헤더 체크박스 — 지금 보이는 행 전체 선택/해제.
  const visibleIds = rows.map((r) => r.personId)
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-6 gap-3">
        <SummaryTile label="명부 총원" value={`${goods.total}명`} sub={`배치 ${people.filter((p) => !p.isReserve).length} · 예비 ${people.filter((p) => p.isReserve).length}`} />
            ...
            {rows.map((p, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5">
                  {p.role === '거점관리자' ? (
                    <span className="rounded-md bg-primary-50 px-1.5 py-0.5 text-caption font-semibold text-primary-700">거점관리자</span>
                  ) : (
